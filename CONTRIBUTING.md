...
1. Implement `sandbox.harness.base.BaseHarness` (`name()`, `features()`, `suite_ids()`, setup/run as in [`docs/paper/SANDBOX_REPO_DESIGN.md`](docs/paper/SANDBOX_REPO_DESIGN.md)).
2. Register in `sandbox/harness/factory.py`:

   ```python
   HarnessFactory._MAP["your-harness"] = "sandbox.harness.your_mod:YourHarness"
   ```

3. A harness that cannot sit a `decide()`-bench **skips**; it does **not** HOLD-fill. Empty `suite_ids()` = sit none.
4. Do not grow `sandbox/runtime/run_suites.py` or `Trial` with `if harness == ...`. Unknown names already raise `ValueError` listing keys.
...
3. Register in `sandbox/benches/factory.py`:
   ...
   ProtocolFactory._MAP["yourbench.suite_id"] = "runners.protocols:yourbench_protocol"
   ```

   `BenchFactory.create` **always** wraps with `EnvAdapterAsBench` (FinMCP uses `FinMcpEnvAdapterAsBench` for `exec_sync` opt-in).
4. Add a protocol helper in `runners/protocols.py` and an optional short alias in `SUITE_ALIASES`.
5. Clone URL goes in `scripts/clone_modules.sh` + `modules/README.md` + `MODULES.md`. Do not vendor the clone.
...
1. Implement `sandbox.plugins.base.EnvPlugin`.
2. Register in `sandbox/plugins/factory.py`:

   ```python
   PluginFactory._MAP["your-plugin"] = "sandbox.plugins.your_mod:YourPlugin"
   ```

3. MCP / Qieman attach here (`McpPlugin` is `plugin_env` for FinMCP). Yahoo harvest stays `runners/yahoo.py` — do **not** register it as a plugin or a `SandboxFactory` type.
...
```bash
./scripts/doctor.sh
PYTHONPATH=. python -m unittest tests.unit.test_factory_stubs \
