...
Known patches and harvest gaps: [`docs/engineering/GLUE.md`](engineering/GLUE.md).
...
Module-level wrap plans live as docstrings in `adapters/{stockbench,ama,deepfund,fintoolbench,finsaber}/adapter.py`.

Optional-suite glue: `adapters/_ops/runtime.py` is the shared subprocess / xAI / skip / patch helper. Each optional `EnvAdapter` stays protocol → official CLI → `SuiteResult`. Harvest, resume, and upstream patches for FinSearchComp / Vals / LiveTradeBench live in `adapters/<bench>/ops.py`. Operator CLI: `cli/optional_suite_ops.py` (`progress|harvest|resume`). Main eval entry remains `cli/run_eval.py` (`cli/run_grok_cli_eval.py` is a thin alias). One scorecard per harness (`reports/GROK_CLI_SCORECARD.*` for grok-cli) with a required/optional tier per suite.

How OpenClaw, Hermes, and each `modules/*` native surface map onto this LCD: [`docs/research/AGENT_PLUG_IN_SURVEY.md`](research/AGENT_PLUG_IN_SURVEY.md), [`docs/research/PLUG_IN_MATRIX.md`](research/PLUG_IN_MATRIX.md).

## FinAgentSandbox registries (pointer)

The four-layer story above is unchanged. A Harbor-style registration layer lives in `src/finagent/` so a second harness, bench, or MCP plugin is a `_MAP` entry rather than a new eval loop. Product name: **FinAgentSandbox**.

Interface sketches: [`docs/paper/SANDBOX_REPO_DESIGN.md`](paper/SANDBOX_REPO_DESIGN.md). Harbor norms: [`docs/paper/HARBOR_NORMS.md`](paper/HARBOR_NORMS.md). Order: [`docs/paper/MIGRATION_PLAN.md`](paper/MIGRATION_PLAN.md) (Phase B = GrokCliHarness + run_suites; Phase C = BenchFactory wrap).
...
Eval runs through `finagent.trial.run_suites` (`cli/run_eval.py --harness grok-cli`; `cli/run_grok_cli_eval.py` is a thin alias). New dumps go under `artifacts/suite_results/<harness>/`. Scorecards and legacy flat `artifacts/suite_results/*.json` are not rewritten here.
